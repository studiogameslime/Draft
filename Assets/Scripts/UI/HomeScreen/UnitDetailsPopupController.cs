...
public class UnitDetailsPopupController : MonoBehaviour
{
    ...
    [SerializeField] private TMP_Text atkSpeedTitle;
    ...
    public void FillData()
    {
        ...
        // ---------- STATS TEXT ----------
        bool isMiner = _unit.id == "unit_miner";
        ...
        // Toggle rows visibility (assuming the Text is inside a parent container like a Row)
        if (hpText != null)
        {
            hpText.text = hpNow.ToString();
            hpText.transform.parent.parent.gameObject.SetActive(!isMiner);
        }

        if (dmgText != null)
        {
            dmgText.text = dmgNow.ToString();
            dmgText.transform.parent.parent.gameObject.SetActive(!isMiner);
        }
        ...
        if (rangeText != null)
        {
            ...
            rangeText.transform.parent.gameObject.SetActive(!isMiner);
        }
        ...
        if (speedText != null)
        {
            ...
            speedText.transform.parent.gameObject.SetActive(!isMiner);
        }

        if (targetPriorityText != null)
        {
            targetPriorityText.text = _unit.targetPriorityClass.ToString();
            targetPriorityText.transform.parent.gameObject.SetActive(!isMiner);
        }

        // Custom Miner display
        if (spawnTimeText != null)
        {
            spawnTimeText.text = isMiner ? "None" : _unit.spawnTime.ToString();
        }
        ...
        if (atkSpeedTitle != null)
        {
            atkSpeedTitle.text = isMiner ? "Mining Speed" : "Hit Speed";
        }
        ...
        if (!atMax)
        {
            ...
            if (addHpText != null)
            {
                ...
                addHpText.gameObject.SetActive(!isMiner); // HIDE FOR MINER
                addHpText.text = $"+{hpAdd}";
            }

            if (addDmgText != null)
            {
                addDmgText.gameObject.SetActive(!isMiner); // HIDE FOR MINER
