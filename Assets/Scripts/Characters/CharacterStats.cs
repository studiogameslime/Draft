...
public class CharacterStats : MonoBehaviour, ICombatTarget
{
    ...
    private void TrySpawnSoulOnDeath()
    {
        ...
        // Base chance is 0.25 (25%) from sources [3, 4]
        float baseChance = Mathf.Clamp01(definition.soulDropChance);

        // Mastery bonus is 0.01 per level (up to 0.20 at level 20)
        float bonusPercent = MasteryBonusManager.Instance != null
            ? MasteryBonusManager.Instance.GetPercent(MasteryStat.SoulsDropPercent)
            : 0f;

        // ADDITIVE CALCULATION: base 25% + mastery 10% = 35%
        // We just add them together instead of multiplying
        float finalChance = Mathf.Clamp01(baseChance + bonusPercent);
